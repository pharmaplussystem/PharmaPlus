...
-- ============================================================
-- PharmaPlus Online Customer Ordering
-- ============================================================
create table if not exists public.customer_profiles (
  id uuid primary key references auth.users(id) on delete cascade,
  full_name text not null default '', phone text, pharmacy_id uuid not null references public.pharmacies(id) on delete cascade,
  address text, active boolean not null default true, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create index if not exists customer_profiles_pharmacy_idx on public.customer_profiles(pharmacy_id);

create table if not exists public.online_orders (
  id uuid primary key default gen_random_uuid(),
  order_number text not null unique default ('ON-' || upper(substr(replace(gen_random_uuid()::text,'-',''),1,8))),
  customer_id uuid not null references public.customer_profiles(id) on delete cascade,
  pharmacy_id uuid not null references public.pharmacies(id) on delete cascade,
  status text not null default 'pending' check(status in ('pending','confirmed','preparing','ready','out_for_delivery','completed','rejected','cancelled')),
  ...
  total numeric(14,2) not null default 0, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), confirmed_at timestamptz
);
create index if not exists online_orders_pharmacy_status_idx on public.online_orders(pharmacy_id,status,created_at desc);
create index if not exists online_orders_customer_idx on public.online_orders(customer_id,created_at desc);
...
create table if not exists public.online_order_items (
  ...
  product_id uuid references public.products(id) on delete set null, product_name text not null, brand_name text, batch text, qty integer not null check(qty>0),
  unit_price numeric(14,2) not null default 0, line_total numeric(14,2) not null default 0, pharmacy_id uuid not null references public.pharmacies(id) on delete cascade,
  created_at timestamptz not null default now()
);
...
drop policy if exists customer_profile_self on public.customer_profiles;
create policy customer_profile_self on public.customer_profiles for select to authenticated using(id=auth.uid());
drop policy if exists customer_profile_update on public.customer_profiles;
...
drop policy if exists online_orders_customer_select on public.online_orders;
create policy online_orders_customer_select on public.online_orders for select to authenticated using(customer_id=auth.uid());
drop policy if exists online_order_items_customer_select on public.online_order_items;
create policy online_order_items_customer_select on public.online_order_items for select to authenticated using(order_id in (select id from public.online_orders where customer_id=auth.uid()));

drop policy if exists online_orders_staff_select on public.online_orders;
create policy online_orders_staff_select on public.online_orders for select to authenticated using(pharmacy_id=public.current_pharmacy() and public.has_role(array['admin','manager','pharmacist','storekeeper','staff']::public.user_role[]));
drop policy if exists online_order_items_staff_select on public.online_order_items;
create policy online_order_items_staff_select on public.online_order_items for select to authenticated using(pharmacy_id=public.current_pharmacy() and public.has_role(array['admin','manager','pharmacist','storekeeper','staff']::public.user_role[]));

create or replace function public.get_online_pharmacies()
returns table(id uuid,name text,location text,contact text,email text,logo_url text,currency text)
language sql security definer set search_path=public as $$
  select p.id,p.name,p.location,p.contact,p.email,p.logo_url,p.currency from public.pharmacies p order by p.name;
$$;

create or replace function public.get_online_catalog(p_pharmacy uuid)
returns table(id uuid,name text,brand_name text,category text,batch text,expiry date,qty integer,retail_price numeric)
language sql security definer set search_path=public as $$
  select p.id,p.name,coalesce(p.brand_name,''),p.category,p.batch,p.expiry,p.qty,coalesce(nullif(p.retail_price,0),p.sell)
...
create or replace function public.handle_new_user()
returns trigger language plpgsql security definer set search_path=public as $function$
declare pid uuid; assigned_role public.user_role := 'staff'; inv record; invite_code text; acct text; cust_pharmacy uuid;
begin
 ...
 if acct='customer' then
   ...
   if cust_pharmacy is null then raise exception 'Please select a pharmacy'; end if;
   if not exists(select 1 from public.pharmacies where id=cust_pharmacy) then raise exception 'Selected pharmacy not found'; end if;
   insert into public.customer_profiles(id,full_name,phone,pharmacy_id,address) values(new.id,coalesce(new.raw_user_meta_data->>'full_name',''),nullif(new.raw_user_meta_data->>'phone',''),cust_pharmacy,nullif(new.raw_user_meta_data->>'address','')) on conflict(id) do update set full_name=excluded.full_name,phone=excluded.phone,pharmacy_id=excluded.pharmacy_id,address=excluded.address,updated_at=now();
   return new;
 end if;
 invite_code := upper(trim(coalesce(new.raw_user_meta_data->>'invite_code','')));
 if invite_code <> '' then
...
create or replace function public.place_online_order(p_pharmacy uuid,p_fulfillment text,p_delivery_address text,p_phone text,p_notes text,p_items jsonb)
returns table(order_id uuid,order_number text,total numeric)
language plpgsql security definer set search_path=public as $$
declare cid uuid; oid uuid:=gen_random_uuid(); onum text:='ON-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,8)); it record; pr record; v_total numeric(14,2):=0;
begin
 ...
 if p_fulfillment not in ('delivery','pickup') then raise exception 'Invalid fulfillment option'; end if;
 if p_fulfillment='delivery' and nullif(trim(p_delivery_address),'') is null then raise exception 'Delivery address is required'; end if;
 if jsonb_array_length(coalesce(p_items,'[]'::jsonb))<1 then raise exception 'Your cart is empty'; end if;
 for it in select * from jsonb_to_recordset(p_items) as x(product_id uuid,qty integer) loop
   select * into pr from public.products where id=it.product_id and pharmacy_id=p_pharmacy and qty>0 and coalesce(archived,false)=false and expiry>=current_date;
   if not found then raise exception 'One of the selected medicines is no longer available'; end if;
   if it.qty is null or it.qty<1 or it.qty>pr.qty then raise exception 'Insufficient stock for %',pr.name; end if;
   v_total:=v_total+(coalesce(nullif(pr.retail_price,0),pr.sell)*it.qty);
 end loop;
 insert into public.online_orders(id,order_number,customer_id,pharmacy_id,status,fulfillment,delivery_address,phone,notes,total) values(oid,onum,cid,p_pharmacy,'pending',p_fulfillment,nullif(trim(p_delivery_address),''),nullif(trim(p_phone),''),nullif(trim(p_notes),''),v_total);
 for it in select * from jsonb_to_recordset(p_items) as x(product_id uuid,qty integer) loop
   select * into pr from public.products where id=it.product_id and pharmacy_id=p_pharmacy;
   insert into public.online_order_items(order_id,product_id,product_name,brand_name,batch,qty,unit_price,line_total,pharmacy_id) values(oid,pr.id,pr.name,coalesce(pr.brand_name,''),pr.batch,it.qty,coalesce(nullif(pr.retail_price,0),pr.sell),coalesce(nullif(pr.retail_price,0),pr.sell)*it.qty,p_pharmacy);
 end loop;
 return query select oid,onum,v_total;
end; $$;

create or replace function public.update_online_order_status(p_order uuid,p_status text)
returns public.online_orders language plpgsql security definer set search_path=public as $$
declare o public.online_orders; it record; pr public.products%rowtype;
begin
 if not public.has_role(array['admin','manager','pharmacist','storekeeper','staff']::public.user_role[]) then raise exception 'Not authorized'; end if;
 select * into o from public.online_orders where id=p_order and pharmacy_id=public.current_pharmacy() for update;
 if o.id is null then raise exception 'Order not found'; end if;
 if p_status not in ('pending','confirmed','preparing','ready','out_for_delivery','completed','rejected','cancelled') then raise exception 'Invalid status'; end if;
 if p_status='confirmed' and o.status='pending' then
   for it in select * from public.online_order_items where order_id=o.id loop
     select * into pr from public.products where id=it.product_id and pharmacy_id=o.pharmacy_id for update;
     if not found or pr.qty<it.qty then raise exception 'Insufficient stock for %',it.product_name; end if;
   end loop;
   for it in select * from public.online_order_items where order_id=o.id loop
     update public.products set qty=qty-it.qty where id=it.product_id and pharmacy_id=o.pharmacy_id;
   end loop;
   update public.online_orders set status='confirmed',confirmed_at=now(),updated_at=now() where id=o.id;
 elsif o.status in ('rejected','cancelled','completed') then raise exception 'This order can no longer be changed';
 else
   update public.online_orders set status=p_status,updated_at=now() where id=o.id;
 end if;
 select * into o from public.online_orders where id=p_order;
 return o;
end; $$;
